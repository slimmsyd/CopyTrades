...
export const ApiSettings: React.FC<ApiSettingsProps> = ({
  ...
  const testEndpoint = async () => {
    try {
      ...
      addNotification(error instanceof Error ? error.message : 'API test failed', 'error');
    }
  };
  ...
  const testWalletTracking = async () => {
    try {
      ...
      if (!settings.wallet?.address) {
        throw new Error('No wallet address configured');
      }

      // Start tracking the wallet
      await apiService.trackWallet(settings.wallet.address);
      addNotification('Started tracking wallet', 'success');
    } catch (error) {
      console.error('Wallet tracking error:', error);
      addNotification(error instanceof Error ? error.message : 'Failed to track wallet', 'error');
    }
  };
