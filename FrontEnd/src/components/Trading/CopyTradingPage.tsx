...
export const CopyTradingPage: React.FC = () => {
  ...
  const { loading: trackedTradesLoading } = useTrackedTrades();
    ...
    try {
      ...
      const tradesArray = Object.entries(data).map(([id, tradeData]) => {
        ...
        return {
          ...
          type: trade.type || 'buy',
          value: trade.value || 0,
          token_address: trade.tokenAddress || trade.token_address,
          token_amount: trade.tokenAmount || trade.token_amount || 0,
          ...
          transactionLink: trade.transactionLink || trade.transaction_link,
          html: trade.html || `<span class='text-${trade.type === 'buy' ? 'green' : 'red'}-500'>${trade.type.toUpperCase()}</span>`,
          buy_price: 0,
          close_price: null,
          current_price: null,
          status: 'active'
      ...
      // Filter trades by currently tracked wallet
      const filteredTrades = tradesArray.filter(trade => trade.wallet_group === trackedWallet);

      setTrades(filteredTrades);
      console.log('Filtered trades:', filteredTrades);
      addNotification(`Loaded ${filteredTrades.length} trades for ${trackedWallet}`, 'success');
    } catch (error) {
  ...
  return (
    <div className="p-8">
      ...
      <div className="mb-8">
        ...
        <ActiveTrades trackedWallet={trackedWallet} />
      </div>
