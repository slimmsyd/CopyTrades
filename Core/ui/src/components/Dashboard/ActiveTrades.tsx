...
export const ActiveTrades: React.FC = () => {
  const { activeTrades, loading, error, updateTrades } = useTrades();
  ...
  const displayedTrades = activeTrades.slice(startIndex, startIndex + ITEMS_PER_PAGE);
  ...
  const profitableCount = activeTrades.filter(trade => trade.profit > 0).length;
        ...
        <div className="p-6 border-b border-gray-800/50">
            ...
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-2 bg-gray-800/50 rounded-lg p-1">
                <button
                  ...
                  className={`px-4 py-1.5 rounded-lg text-sm transition-colors text-gray-400 hover:text-white`}
                >
              ...
              <div className="text-sm">
                <span className={`text-green-400 font-semibold`}>
                  {activeTrades.reduce((acc, trade) => acc + trade.profit, 0).toFixed(2)} SOL
                </span>
                <span className="text-gray-400 mx-2">•</span>
                <span className="text-blue-400">{(profitableCount / activeTrades.length * 100).toFixed(1)}% Win Rate</span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-4 gap-4">
            <div className="bg-gray-800/50 rounded-lg p-4">
              ...
              <div className="text-white font-semibold">{activeTrades.length}</div>
            </div>
            <div className="bg-gray-800/50 rounded-lg p-4">
            ...
            <div className="bg-gray-800/50 rounded-lg p-4">
              ...
              <div className="text-red-400 font-semibold">{activeTrades.length - profitableCount}</div>
            </div>
            <div className="bg-gray-800/50 rounded-lg p-4">
              <div className="text-gray-400 text-sm mb-1">Total Profit</div>
              <div className={`font-semibold text-green-400`}>
                {activeTrades.reduce((acc, trade) => acc + trade.profit, 0).toFixed(4)} SOL
              </div>
            </div>
          </div>
        </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-sm text-gray-400">
                      Entry: {trade.buy_price.toFixed(8)} SOL
                    </div>
                    <div className="text-sm text-gray-400">
                      Current: {trade.current_price ? trade.current_price.toFixed(8) : 'Loading...'} SOL
                    </div>
                    <div className="text-sm text-gray-400">
